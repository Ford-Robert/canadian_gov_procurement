...
Data was analyzed through the R programming software [@citeR], and `RSelenium` [TODO] with `rvest` [TODO] were used to support a web scraping script to download all data. Additional packages such as `tidyverse` [@tidyverse], `knitr` [@knitr], `actuar` [TODO], `readr` [TODO], `stringr` [TODO], `broom` [TODO] and `dplyr` [@dplyr] were used to help clean, simulate, analyze, and test the data. The presentation of this data using tables and graphs was made possible by r-packages; `ggplot2` [TODO], `lubridate` [TODO], `kableExtra` [TODO], and `zoo` [TODO].

A web scraping script was used to collect all Federal Award Data from the IJF [REF]. The variables this paper focuses are; Contract, Buyer, Supplier, Amount, Award Date, Start Date, and End Date.  
...
- Dplyr: For data manipulation and transformation [REF] 
- Car: For advanced regression modeling tools and diagnostics [REF] 
- Caret: For classification and regression training, including model tuning and validation [REF] 
...
Talk about what I did thoroughly, see data_cleaning script
